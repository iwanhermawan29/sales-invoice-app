<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl">{{ __('Edit Sales Order') }}</h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="p-6 bg-white rounded-lg shadow">
                <form action="{{ route('sales-orders.update', $salesOrder) }}" method="POST" class="space-y-6"
                    x-data="{ lines: {{ json_encode(old('lines', $salesOrder->items->map(fn($i) => ['item_id' => $i->item_id, 'qty' => $i->quantity, 'unit_price' => $i->unit_price])->toArray())) }} }">
                    @csrf @method('PUT')
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <x-input-label for="so_number" :value="__('SO Number')" />
                            <x-text-input id="so_number" name="so_number" type="text"
                                value="{{ old('so_number', $salesOrder->so_number) }}" readonly
                        ...
                        <div>
                            ...
                            <x-text-input id="order_date" name="order_date" type="date"
                                value="{{ old('order_date', \Carbon\Carbon::parse($salesOrder->order_date)->toDateString()) }}"
                    ...
                    <div class="space-y-2">
                            ...
                            <div class="flex space-x-2 items-end">
                                <div class="flex-1">
                                    ...
                                    <select x-bind:id="'item_' + idx" x-bind:name="'lines[' + idx + '][item_id]'"
                                        ...
                                        @foreach ($items as $it)
                                            <option value="{{ $it->id }}"
                                                x-bind:selected="line.item_id == {{ $it->id }}">
                                                {{ $it->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <button type="button" @click="lines.push({item_id:'',qty:1,unit_price:0})"
                            class="text-blue-600">+ Add Line</button>
                    </div>

                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('sales-orders.index') }}"
                            class="text-gray-600 hover:underline">{{ __('Batal') }}</a>
                        <x-primary-button>{{ __('Update SO') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
